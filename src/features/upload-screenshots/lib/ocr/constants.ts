...
export const DATE_HEADER_REGEX =
  /^(\d{1,2})\s+(янв(?:аря)?|фев(?:раля)?|мар(?:та)?|апр(?:еля)?|мая?|июн(?:я)?|июл(?:я)?|авг(?:уста)?|сен(?:тября)?|окт(?:ября)?|ноя(?:бря)?|дек(?:абря)?)\.?(?:,?\s*[а-яё]{2})?(?:\s+[+\-]?\s?(?:\d{1,3}(?:[ .]\d{3})+|\d+)(?:[,.]\d{1,2})?\s*(?:₽|руб\.?|rub|rur|€|eur|\$|usd|[РPp])?)?$/i;
